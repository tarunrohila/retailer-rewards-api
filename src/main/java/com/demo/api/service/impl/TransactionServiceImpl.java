...
/**
 * Implementation for @{@link TransactionService} which is used to handle requests for Customer Transactions
 *
 * @author Tarun Rohila
 * @since Mar 25, 2023
 */
...
public class TransactionServiceImpl implements TransactionService {
    ...
    public TransactionResponse addTransaction(TransactionRequest transactionDto) {
        LOGGER.debug("Adding a transaction = [{}] for customerId = [{}] in TransactionServiceImpl.addTransaction", transactionDto, transactionDto.getCustomerId());
        Transaction transaction = Transaction.builder()
                .purchaseAmount(transactionDto.getPurchaseAmount())
                .customer(customerDao.retrieveCustomer(transactionDto.getCustomerId())).build();
        LOGGER.debug("Copying transaction properties from TransactionDto = [{}] to TransactionEntity = [{}] Object", transactionDto, transaction);
        transaction.setPointsEarned(calculatePointsForTransaction(transactionDto.getPurchaseAmount()));
        Transaction createdTransaction = transactionDao.addTransaction(transaction);
        LOGGER.info("New Customer Transaction = [{}] is added for customerId = [{}]", createdTransaction, transaction.getCustomer().getId());
        LOGGER.debug("Calculated Earned Point for the transaction are {} for customerId = [{}]", createdTransaction.getPointsEarned(), transaction.getCustomer().getId());
        return TransactionResponse
                ...
                .id(createdTransaction.getId()).build();
    }

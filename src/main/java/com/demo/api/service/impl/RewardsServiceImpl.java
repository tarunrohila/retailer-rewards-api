...
/**
 * Implementation for @{@link RewardsService} which is used to provide handling for methods for
 * Retailer's Reward scheme
 *
 * @author Tarun Rohila
 * @since Mar 25, 2023
 */
...
public class RewardsServiceImpl implements RewardsService {
    ...
    public RewardsStatementDto retrieveRewardsForCustomer(Long customerId) {
        LOGGER.debug(
                "retrieving rewards for customerId = [{}] in RewardsServiceImpl.retrieveRewardsForCustomer",
                customerId);
        ...
        rewardsStatementDto.setCustomerId(customerId);
        transactions
                .stream()
                .forEach(
                        transaction -> {
                            LOGGER.debug(
                                    "Adding rewards points for last three months for transaction id = [{}]",
                                    transaction.getCustomer().getId());
                            rewardsStatementDto.setCustomerName(
                                    transaction.getCustomer().getCustomerName());
                            rewardsStatementDto.setTotalPoints(
                                    rewardsStatementDto.getTotalPoints()
                                            + transaction.getPointsEarned());
                            rewardsStatementDto.setTotalPurchase(
                                    rewardsStatementDto.getTotalPurchase()
                                            + transaction.getPurchaseAmount());
                            if (rewardsStatementDto
                                    .getPointsByMonth()
                                    .containsKey(transaction.getCreateDate().getMonth().name())) {
                                rewardsStatementDto
                                        .getPointsByMonth()
                                        .put(
                                                transaction.getCreateDate().getMonth().name(),
                                                rewardsStatementDto
                                                                .getPointsByMonth()
                                                                .get(
                                                                        transaction
